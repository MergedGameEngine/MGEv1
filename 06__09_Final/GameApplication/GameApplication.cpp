...
//		SetupRC(hDC);
		Init_ALL();
		ant = new ANCSync();
		ant->AddController(*ac);
		ant->AddController(*wac);
		ant->hWnd = hWnd;
		ant->hDC = hDC;
		ant->interval = 16;
		ant->Start();
	...
	case WM_KEYDOWN:
		{
			...
			TMessage tmsg;
			tmsg.message = TWindow::GEM_KEYDOWN;
	...
	case WM_MOUSEMOVE:
		{
			...
			_y = (short)(lParam >> 16);
			TMessage tmsg;
			tmsg.message = TWindow::GEM_MOUSEMOVE;
	...
	case 0x020A:
		{
			...
			TMessage tmsg;
			tmsg.message = TWindow::GEM_MOUSEWHEEL;
