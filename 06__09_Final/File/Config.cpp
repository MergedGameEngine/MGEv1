...
char _FILE_API *TConfig::GetClassName() {
	return "TConfig";
}
int _FILE_API TConfig::GetClassCode() {
	return CLSID_TConfig;
}
int _FILE_API TConfig::GetHashCode() {
	return 1;
}
BOOL32 _FILE_API TConfig::IsInstanceof(const char* className) {
	if ( strcmp("TConfig", className) == 0 ) {
		return TRUE;
	}
...
BOOL32 _FILE_API TConfig::IsEqualto(IObject &obj) {
	return FALSE;
}
void _FILE_API TConfig::Serialize(IIOStream &iio) {
}
void _FILE_API TConfig::Wait() {
}
void _FILE_API TConfig::Notify() {
}

TRefArray<TString> _FILE_API &TConfig::GetNames() {
	return names;
}
TRefArray<TString> _FILE_API &TConfig::GetValues() {
	return values;
}
THashtable _FILE_API &TConfig::GetNameIndices() {
	return nameIndices;
}
...
double _FILE_API TConfig::StringtoDouble(TString &src) {
	unsigned short *stream = (unsigned short *)src.GetBuffer();
...
BOOL32 _FILE_API TConfig::ParseInt(TString &key, int &des) {
	TString *result = (TString*)nameIndices.Get(key);
	if ( result != NULL ) {
		unsigned short *stream = (unsigned short *)result->GetBuffer();
		unsigned int i = 0;
		unsigned short achar = 0;
...
BOOL32 _FILE_API TConfig::ParseDouble(TString &key, double &des) {
	TString *result = (TString*)nameIndices.Get(key);
	if ( result != NULL ) {
		unsigned short *stream = (unsigned short *)result->GetBuffer();
		unsigned int i = 0;
...
BOOL32 _FILE_API TConfig::ParseVector3f(TString &key, Vector3f &des) {
	TString *result = (TString*)nameIndices.Get(key);
	if ( result != NULL ) {
		Vector3f desc;
		unsigned short *stream = (unsigned short *)result->GetBuffer();
...
BOOL32 _FILE_API TConfig::ParseVector4f(TString &key, Vector4f &des) {
	TString *result = (TString*)nameIndices.Get(key);
	if ( result != NULL ) {
		Vector4f desc;
		unsigned short *stream = (unsigned short *)result->GetBuffer();
...
BOOL32 _FILE_API TConfig::ParseMatrix33f(TString &key, Matrix33f &des) {
	TString *result = (TString*)nameIndices.Get(key);
	if ( result != NULL ) {
		Matrix33f desc;
		unsigned short *stream = (unsigned short *)result->GetBuffer();
...
BOOL32 _FILE_API TConfig::ParseMatrix44f(TString &key, Matrix44f &des) {
	TString *result = (TString*)nameIndices.Get(key);
	if ( result != NULL ) {
		Matrix44f desc;
		unsigned short *stream = (unsigned short *)result->GetBuffer();
...
BOOL32 _FILE_API TConfig::GetString(TString &key, TString &des) {
	TString *result = (TString*)nameIndices.Get(key);
	if ( result != NULL ) {
		des = *result;
		return TRUE;
