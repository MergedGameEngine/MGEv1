...
const unsigned long Blowfish::ORIG_P[16 + 2] = {
...
const unsigned long Blowfish::ORIG_S[4][256] = {
...
_NETWORK_API Blowfish::Blowfish() {
}
_NETWORK_API Blowfish::~Blowfish() {
}

unsigned long _NETWORK_API Blowfish::F(unsigned long x) {
...
void _NETWORK_API Blowfish::Encrypt(unsigned long *xl, unsigned long *xr){
	unsigned long  Xl;
	unsigned long  Xr;
	unsigned long  temp;
	short       i;
...
void _NETWORK_API Blowfish::Decrypt(unsigned long *xl, unsigned long *xr){
	unsigned long  Xl;
	unsigned long  Xr;
	unsigned long  temp;
	short       i;
...
void _NETWORK_API Blowfish::Init(unsigned char *key, int keyLen) {
	int i, j, k;
...
void _NETWORK_API Blowfish::SwapEndian(unsigned long *src) {
	unsigned char *buf = (unsigned char *)src;
	unsigned char temp = 0;
	temp = buf[3]; buf[3] = buf[0]; src[0] = temp;
	temp = buf[2]; buf[2] = buf[1]; src[1] = temp;
}

void _NETWORK_API Blowfish::Encrypt(const void *buf, unsigned int llen) {
	unsigned int len = llen - 1;
	unsigned long *lbuf = (unsigned long *)buf;
	unsigned long *L = lbuf;
	unsigned long *R = lbuf;
...
void _NETWORK_API Blowfish::Decrypt(const void *buf, unsigned int llen) {
	unsigned long *lbuf = (unsigned long *)buf;
	unsigned long *L = lbuf;
	unsigned long *R = lbuf;
	for ( int i1 = (int)(llen - 2) ; i1 >= 0 ; i1 -- ) {
		L = &(lbuf[i1]);
		R = &(lbuf[i1 + 1]);
		Decrypt(L, R);
	}
